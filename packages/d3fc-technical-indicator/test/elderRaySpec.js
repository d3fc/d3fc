...
describe('elderRay', () => {
    it('should match the expected output', (done) => {
        Promise.all([readCsv('input.csv'), readCsv('elderRay.csv')])
            .then((result) => {
                ...
                expect(output.map((d) => d.bullPower)).toBeEqualWithTolerance(
                    expectedOutput.map((d) => d.BULL),
                );
                expect(output.map((d) => d.bearPower)).toBeEqualWithTolerance(
                    expectedOutput.map((d) => d.BEAR),
                );
            })
            .then(done, done.fail);
    });
