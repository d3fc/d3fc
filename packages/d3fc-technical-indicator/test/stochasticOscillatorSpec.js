...
describe('stochasticOscillator', () => {
    it('should match the expected output', (done) => {
        Promise.all([readCsv('input.csv'), readCsv('stochastic.csv')])
            .then((result) => {
                ...
                expect(output.map((d) => d.k)).toBeEqualWithTolerance(
                    expectedOutput.map((d) => d.K),
                );
                expect(output.map((d) => d.d)).toBeEqualWithTolerance(
                    expectedOutput.map((d) => d.D),
                );
            })
            .then(done, done.fail);
    });
