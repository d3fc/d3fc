...
export default function () {
    ...
    const wildersSmoothing = (values, prevAvg) =>
        prevAvg + (values[values.length - 1] - prevAvg) / values.length;
    const downChange = ([prevClose, close]) =>
        prevClose < close ? 0 : prevClose - close;
    const upChange = ([prevClose, close]) =>
        prevClose > close ? 0 : close - prevClose;

    const updateAverage = (changes, prevAverage) =>
        prevAverage !== undefined
            ? wildersSmoothing(changes, prevAverage)
            : mean(changes);
    ...
    const makeAccumulator = () => {
        ...
        return (closes) => {
            ...
            downChangesAvg = updateAverage(
                closePairs.map(downChange),
                downChangesAvg,
            );
            upChangesAvg = updateAverage(
                closePairs.map(upChange),
                upChangesAvg,
            );
