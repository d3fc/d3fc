...
export const base = (nonTradingPattern, dateTimeUtility) => {
    const getDayPatternOrDefault = (day) =>
        nonTradingPattern[day] === undefined ? [] : nonTradingPattern[day];
    ...
    const totalTradingWeekMilliseconds = tradingDays.reduce(
        (total, tradingDay) => total + tradingDay.totalTradingTimeInMiliseconds,
        0,
    );
        ...
        for (const range of tradingDay.nonTradingTimeRanges) {
            if (range.isInRange(date)) {
                return range.startTime === dayBoundary
                    ? instance.clampDown(
                          dateTimeUtility.getEndOfPreviousDay(date),
                      )
    ...
    instance.distance = (startDate, endDate) => {
        ...
        // same day distance
        if (
            dateTimeUtility.dayInterval(start).getTime() ===
            dateTimeUtility.dayInterval(end).getTime()
        ) {
            return instance.tradingDays[
                dateTimeUtility.getDay(start)
            ].totalTradingMillisecondsBetween(start, end);
        }

        // combine any trading time left in the day after startDate
        // and any trading time from midnight up until the endDate
        let total =
            instance.tradingDays[
                dateTimeUtility.getDay(start)
            ].totalTradingMillisecondsBetween(
                start,
                dateTimeUtility.dayInterval.offset(
                    dateTimeUtility.dayInterval(start),
                    1,
                ),
            ) +
            instance.tradingDays[
                dateTimeUtility.getDay(end)
            ].totalTradingMillisecondsBetween(
                dateTimeUtility.dayInterval(end),
                end,
            );
        ...
        start = dateTimeUtility.dayInterval.offset(
            dateTimeUtility.dayInterval(start),
            1,
        );
        ...
        return (
            ...
            dateTimeUtility.dayInterval
                ...
                .reduce((runningTotal, currentDay, currentIndex, arr) => {
                    ...
                    const tradingDay =
                        instance.tradingDays[
                            dateTimeUtility.getDay(currentDay)
                        ];
                    return (runningTotal += isDstBoundary
                        ? tradingDay.totalTradingMillisecondsBetween(
                              currentDay,
                              nextDay,
                          )
    ...
    instance.offset = (date, ms) => {
        ...
        const isDstBoundary = (d) =>
            dateTimeUtility.dayInterval.offset(d) -
                dateTimeUtility.dayInterval(d) !==
            millisPerDay;

        const moveToDayBoundary = (tradingDay, date, ms) => {
            if (ms < 0) {
                const dateFloor = dateTimeUtility.dayInterval(date);
                const distanceToStartOfDay =
                    tradingDay.totalTradingMillisecondsBetween(dateFloor, date);

                return Math.abs(ms) <= distanceToStartOfDay
                    ? tradingDay.offset(date, ms)
                    : [
                          instance.clampDown(
                              dateTimeUtility.msInterval.offset(dateFloor, -1),
                          ),
                          ms + distanceToStartOfDay + 1,
                      ];
            } else {
                const nextDate = dateTimeUtility.getStartOfNextDay(date);
                const distanceToDayBoundary =
                    tradingDay.totalTradingMillisecondsBetween(date, nextDate);
        ...
        const moveDateDelegate =
            ms < 0
                ? (date, remainingMs, tradingDayMs) => [
                      instance.clampDown(
                          dateTimeUtility.dayInterval.offset(date, -1),
                      ),
                      remainingMs + tradingDayMs,
                  ]
                : (date, remainingMs, tradingDayMs) => [
                      instance.clampUp(
                          dateTimeUtility.dayInterval.offset(date),
                      ),
                      remainingMs - tradingDayMs,
                  ];
        ...
        while (ms !== 0) {
            ...
            if (isDstBoundary(date)) {
                ...
                [date, ms] =
                    Math.abs(ms) >= tradingDay.totalTradingTimeInMiliseconds
                        ? moveDateDelegate(
                              date,
                              ms,
                              tradingDay.totalTradingTimeInMiliseconds,
...
export default (nonTradingHoursPattern) =>
    base(nonTradingHoursPattern, localDateTimeUtility);
