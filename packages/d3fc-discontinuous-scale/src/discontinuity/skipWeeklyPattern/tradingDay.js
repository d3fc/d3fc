...
export const tradingDay = (rawDiscontinuityTimeRanges, dateTimeUtility) => {
    ...
    const totalTradingTimeInMiliseconds =
        millisPerDay -
        nonTradingTimeRanges.reduce(
            (total, range) => total + range.lenghtInMs,
            0,
        );
    ...
    const totalTradingMillisecondsBetween = (intervalStart, intervalEnd) => {
        ...
        if (
            dateTimeUtility.dayInterval(intervalStart).getTime() !==
                dateTimeUtility.dayInterval(intervalEnd).getTime() &&
            dateTimeUtility.getStartOfNextDay(intervalStart).getTime() !==
                intervalEnd.getTime()
        ) {
        ...
        const relevantDiscontinuityRanges = nonTradingTimeRanges.filter(
            (range) => {
                return (
                    range.endTime === dayBoundary ||
                    dateTimeUtility.setTime(intervalStart, range.endTime) >=
                        intervalStart
                );
            },
        );
        ...
        for (const nonTradingRange of relevantDiscontinuityRanges) {
            const nonTradingStart = dateTimeUtility.setTime(
                intervalStart,
                nonTradingRange.startTime,
            );
            const nonTradingEnd =
                nonTradingRange.endTime === dayBoundary
                    ? dateTimeUtility.getStartOfNextDay(intervalStart)
                    : dateTimeUtility.setTime(
                          intervalStart,
                          nonTradingRange.endTime,
                      );

            // both intervalStart and intervalEnd are before the start of this non-trading range
            if (
                intervalStart < nonTradingStart &&
                intervalEnd < nonTradingStart
            ) {
                return (
                    total +
                    dateTimeUtility.msInterval.count(intervalStart, intervalEnd)
                );
            }

            // intervalStart is before the start of this non-trading time range
            if (intervalStart < nonTradingStart) {
                total += dateTimeUtility.msInterval.count(
                    intervalStart,
                    nonTradingStart,
                );
            }
        ...
        return (
            total + dateTimeUtility.msInterval.count(intervalStart, intervalEnd)
        );
    };
    ...
    const offset = (date, ms) => {
        ...
        const nonTradingRanges =
            ms > 0
                ? nonTradingTimeRanges.filter(
                      (range) =>
                          dateTimeUtility.setTime(date, range.startTime) >=
                          date,
                  )
        ...
        if (ms > 0) {
            for (const nonTradingRange of nonTradingRanges) {
                const rangeStart = dateTimeUtility.setTime(
                    date,
                    nonTradingRange.startTime,
                );
            ...
            for (const nonTradingRange of nonTradingRanges) {
                ...
                if (offsetDate < endTime) {
                    ...
                    date = dateTimeUtility.msInterval.offset(
                        dateTimeUtility.setTime(
                            date,
                            nonTradingRange.startTime,
                        ),
