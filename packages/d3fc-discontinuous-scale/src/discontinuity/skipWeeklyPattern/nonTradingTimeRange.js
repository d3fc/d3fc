...
export function standardiseTimeString(timeString) {
    ...
    result[0] = isPositiveIntegerUpTo(parseInt(time_components[0], 10), 23)
        ...
        : (function () {
              throw `'Hours' component must be an int between 0 and 23, but was '${time_components[0]}'`;
          })();

    result[1] = isPositiveIntegerUpTo(parseInt(time_components[1], 10), 59)
        ? parseInt(time_components[1], 10)
        : (function () {
              throw `'Minutes' component must be an int between 0 and 59, but was '${time_components[1]}'`;
          })();

    if (time_components.length === 3) {
        const ms_components = time_components[2]
            .split('.')
            .map((x) => parseInt(x, 10));
        ...
        result[2] = isPositiveIntegerUpTo(ms_components[0], 59)
            ? ms_components[0]
            : (function () {
                  throw `'Seconds' component must be an int between 0 and 59, but was '${ms_components[0]}'`;
              })();
        ...
        if (ms_components.length === 2) {
            result[3] = isPositiveIntegerUpTo(ms_components[1], 999)
                ? ms_components[1]
                : (function () {
                      throw `'Miliseconds' component must be an int between 0 and 999, but was '${ms_components[1]}'`;
                  })();
        }
    }
...
export function nonTradingTimeRange(timeRangeTuple, dateTimeUtility) {
    ...
    const lenghtInMs =
        dateTimeUtility.setTime(
            new Date(endTime === dayBoundary ? millisPerDay : 0),
            endTime,
        ) - dateTimeUtility.setTime(new Date(0), startTime);
