...
export const dateTimeUtility = (
    setTimeForDate,
    getDay,
    getTimeComponentArray,
    dayInterval,
    msInterval,
) => {
    const utility = {};
    utility.getTimeComponentArrayFromString = (timeString) => [
        timeString.slice(0, 2),
        timeString.slice(3, 5),
        timeString.slice(6, 8),
        timeString.slice(9, 12),
    ];
    /**
    ...
    utility.getTimeString = (date) => {
        const [hh, mm, ss, ms] = getTimeComponentArray(date).map((x) =>
            x.toString(10).padStart(2, '0'),
        );
    ...
    utility.setTime = (date, timeString, offsetInmilliSeconds = 0) => {
        const [hh, mm, ss, ms] =
            utility.getTimeComponentArrayFromString(timeString);
        return msInterval.offset(
            setTimeForDate(date, hh, mm, ss, ms),
     ...
     * @returns {Date}.
     */
    utility.getStartOfNextDay = (date) =>
        dayInterval.offset(dayInterval.floor(date), 1);
     ...
     * @returns {Date}.
     */
    utility.getEndOfPreviousDay = (date) =>
        msInterval.offset(dayInterval.floor(date), -1);
