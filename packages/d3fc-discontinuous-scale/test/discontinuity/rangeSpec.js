...
describe('discontinuityRange', () => {
    describe('clampUp', () => {
        ...
        it('should clamp up if the number falls within a discontinuity', () => {
            ...
            expect(discontinuityRange([0, 10], [20, 30]).clampUp(25)).toEqual(
                30,
            );
            expect(discontinuityRange([-10, 10], [20, 30]).clampUp(0)).toEqual(
                10,
            );
        });
    ...
    describe('clampDown', () => {
        ...
        it('should clamp down if the number falls within a discontinuity', () => {
            ...
            expect(discontinuityRange([0, 10], [20, 30]).clampDown(25)).toEqual(
                20,
            );
            expect(
                discontinuityRange([-10, 10], [20, 30]).clampDown(0),
            ).toEqual(-10);
        });
    ...
    describe('distance', () => {
        ...
        it('should remove discontinuities', () => {
            ...
            expect(
                discontinuityRange([0, 10], [10, 20]).distance(0, 30),
            ).toEqual(10);
            expect(
                discontinuityRange([0, 10], [20, 30]).distance(0, 30),
            ).toEqual(10);
        });
    });
    ...
    it('should support dates', () => {
        ...
        expect(
            range.distance(new Date(2015, 0, 8), new Date(2015, 0, 11)),
        ).toEqual(millisPerDay * 2);
        expect(range.offset(new Date(2015, 0, 8), millisPerDay * 2)).toEqual(
            new Date(2015, 0, 11),
        );
    });
    ...
    describe('offset', () => {
        ...
        it('should offset with discontinuities', () => {
            ...
            expect(discontinuityRange([5, 10], [20, 30]).offset(0, 40)).toEqual(
                55,
            );
        });
        ...
        it('should allow negative offsets with discontinuities', () => {
            ...
            // multiple discontinuities
            expect(
                discontinuityRange([5, 10], [20, 30]).offset(55, -40),
            ).toEqual(0);
        });
    });
});
