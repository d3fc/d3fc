...
describe('skipWeeklyPattern', () => {
    ...
    describe('clampUp', () => {
        it('should do nothing 1ms before non trading period', () => {
            const expected = timeMillisecond.offset(
                mondayFirstStartBoundary,
                -1,
            );
        ...
        it('should advance to single trading ms', () => {
            ...
            expect(sut.clampUp(new Date(2018, 0, 1))).toEqual(
                new Date(2018, 0, 7, 23, 59, 59, 999),
            );
            expect(sut.clampUp(new Date(2018, 0, 2))).toEqual(
                new Date(2018, 0, 7, 23, 59, 59, 999),
            );
            expect(sut.clampUp(new Date(2018, 0, 3))).toEqual(
                new Date(2018, 0, 7, 23, 59, 59, 999),
            );
            expect(sut.clampUp(new Date(2018, 0, 4))).toEqual(
                new Date(2018, 0, 7, 23, 59, 59, 999),
            );
            expect(sut.clampUp(new Date(2018, 0, 5))).toEqual(
                new Date(2018, 0, 7, 23, 59, 59, 999),
            );
            expect(sut.clampUp(new Date(2018, 0, 6))).toEqual(
                new Date(2018, 0, 7, 23, 59, 59, 999),
            );
            expect(sut.clampUp(new Date(2018, 0, 7))).toEqual(
                new Date(2018, 0, 7, 23, 59, 59, 999),
            );
        });
    });
    ...
    describe('clampDown', () => {
        ...
        it('should clamp down from Sunday 6:59:59.999pm to 1ms before Friday 13:20', () => {
            const expected = timeMillisecond.offset(
                fridaySecondStartBoundary,
                -1,
            );
            const actual = sut.clampDown(
                timeMillisecond.offset(sundayEndBoundary, -1),
            );
            expect(actual).toEqual(expected);
        });
        ...
        it('should clamp down to single trading ms', () => {
            ...
            expect(sut.clampDown(new Date(2018, 0, 1, 0, 0, 0, 1))).toEqual(
                expected,
            );
    ...
    describe('distance', () => {
        it('should return totalTradingWeekMilliseconds', () => {
            expect(
                sut.distance(new Date(2018, 0, 1), new Date(2018, 0, 8)),
            ).toBe(sut.totalTradingWeekMilliseconds);
        });

        it('should return 52 * totalTradingWeekMilliseconds', () => {
            expect(
                sut.distance(new Date(2018, 0, 1), new Date(2018, 11, 31)),
            ).toBe(52 * sut.totalTradingWeekMilliseconds);
        });

        it('should return negative 52 * totalTradingWeekMilliseconds', () => {
            expect(
                sut.distance(new Date(2018, 11, 31), new Date(2018, 0, 1)),
            ).toBe(-52 * sut.totalTradingWeekMilliseconds);
        });
        ...
        it('should return 52 * 7 * 24 * 3600 * 1000 for trading week without non-trading periods', () => {
            ...
            expect(
                sut.distance(new Date(2018, 0, 1), new Date(2018, 11, 31)),
            ).toBe(52 * 7 * 24 * 3600 * 1000);
        });

        it('should return 0 between consecutive non-trading ranges spanning multiple days', () => {
            expect(
                sut.distance(fridaySecondStartBoundary, sundayEndBoundary),
            ).toEqual(0);
            expect(
                sut.distance(sundayEndBoundary, fridaySecondStartBoundary),
            ).toEqual(-0);
        });
        ...
        it('should return -1', () => {
            ...
            expect(
                sut.distance(new Date(2018, 0, 8), new Date(2018, 0, 1)),
            ).toEqual(expected);
            expect(
                sut.distance(new Date(2018, 0, 9), new Date(2018, 0, 2)),
            ).toEqual(expected);
            expect(
                sut.distance(new Date(2018, 0, 10), new Date(2018, 0, 3)),
            ).toEqual(expected);
            expect(
                sut.distance(new Date(2018, 0, 11), new Date(2018, 0, 4)),
            ).toEqual(expected);
            expect(
                sut.distance(new Date(2018, 0, 12), new Date(2018, 0, 5)),
            ).toEqual(expected);
            expect(
                sut.distance(new Date(2018, 0, 13), new Date(2018, 0, 6)),
            ).toEqual(expected);
            expect(
                sut.distance(new Date(2018, 0, 14), new Date(2018, 0, 7)),
            ).toEqual(expected);
        });
    ...
    instead it returns 55min i.e. the non-trading period 1:55-2:00 is only counted once rather than twice`, () => {
        ...
        it('on DST boundaries should return 22hr or 24hr in single non-trading hour in day', () => {
            ...
            expect(
                sut.distance(new Date(2022, 2, 27), new Date(2022, 2, 28)),
            ).toBe(22 * 3600 * 1000);
            expect(
                sut.distance(new Date(2022, 9, 30), new Date(2022, 9, 31)),
            ).toBe(24 * 3600 * 1000);
        });
    });
    ...
    describe('offset', () => {
        it('0 offset should return same date', () => {
            expect(sut.offset(new Date(2018, 0, 1), 0)).toEqual(
                new Date(2018, 0, 1),
            );
        });

        it('0 offset in non-trading range clamps up', () => {
            expect(sut.offset(new Date(2018, 0, 1, 7, 45), 0)).toEqual(
                new Date(2018, 0, 1, 8, 30),
            );
        });

        it('should return end of previous day when offset = -1ms on day boundary ', () => {
            expect(sut.offset(new Date(2018, 0, 1), -1)).toEqual(
                new Date(2017, 11, 31, 23, 59, 59, 999),
            );
        });

        it('-2ms offset should return end of previous day - 1ms', () => {
            expect(sut.offset(new Date(2018, 0, 1), -2)).toEqual(
                new Date(2017, 11, 31, 23, 59, 59, 998),
            );
        });

        it('-1ms offset after 1ms into trading day should return start of day', () => {
            expect(sut.offset(new Date(2018, 0, 1, 0, 0, 0, 1), -1)).toEqual(
                new Date(2018, 0, 1),
            );
        });

        it('should return 1ms before the start of this period when offset = -1ms  at the end of non-trading period', () => {
            expect(sut.offset(new Date(2018, 0, 1, 8, 30), -1)).toEqual(
                new Date(2018, 0, 1, 7, 44, 59, 999),
            );
        });

        it('should return start of next day when offset is 24hr', () => {
            const sut = skipWeeklyPattern({});
            expect(sut.offset(new Date(2018, 0, 1), 24 * 3600 * 1000)).toEqual(
                new Date(2018, 0, 2),
            );
        });
        ...
        it('should return start of previous day when offset is -24hr', () => {
            const sut = skipWeeklyPattern({});
            expect(sut.offset(new Date(2018, 0, 2), -24 * 3600 * 1000)).toEqual(
                new Date(2018, 0, 1),
            );
        });

        it('should return end of non-trading period when offset is 1ms at start of non-trading period', () => {
            expect(sut.offset(new Date(2018, 0, 1, 7, 45), 1)).toEqual(
                new Date(2018, 0, 1, 8, 30, 0, 1),
            );
        });
        ...
        it('should return start of next week when offset is totalTradingWeekMilliseconds', () => {
            expect(
                sut.offset(
                    new Date(2018, 0, 1),
                    sut.totalTradingWeekMilliseconds,
                ),
            ).toEqual(new Date(2018, 0, 8));
        });
        ...
        it('should return start of 53rd week when offset is 52 * totalTradingWeekMilliseconds', () => {
            expect(
                sut.offset(
                    new Date(2018, 0, 1),
                    52 * sut.totalTradingWeekMilliseconds,
                ),
        ...
        it('should return end of second non-trading range when offset covers entire trading period sandwiched between 2 non-trading ones', () => {
            const offset = timeMillisecond.count(
                new Date(2018, 0, 1, 8, 30),
                new Date(2018, 0, 1, 13, 20),
            );
            expect(sut.offset(new Date(2018, 0, 1, 7, 45), offset)).toEqual(
                new Date(2018, 0, 1, 19, 0, 0, 0),
            );
        });

        it('should return 1ms before Friday 13:20 when offset = -1ms on Sunday 7:00pm', () => {
            expect(sut.offset(sundayEndBoundary, -1)).toEqual(
                timeMillisecond.offset(fridaySecondStartBoundary, -1),
            );
        });
