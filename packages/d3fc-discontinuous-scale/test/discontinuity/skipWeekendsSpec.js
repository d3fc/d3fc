...
describe('skipWeekends', () => {
    ...
    describe('clampUp', () => {
        ...
        it('should clamp up if given a weekend', () => {
            ...
            expect(
                skipWeekends().clampUp(
                    timeMillisecond.offset(mondayBoundary, -1),
                ),
            ).toEqual(mondayBoundary);

            // try subtracting a minute
            expect(
                skipWeekends().clampUp(timeMinute.offset(mondayBoundary, -1)),
            ).toEqual(mondayBoundary);

            // try subtracting a day
            expect(
                skipWeekends().clampUp(timeDay.offset(mondayBoundary, -1)),
            ).toEqual(mondayBoundary);

            // try subtracting one and a half days
            expect(
                skipWeekends().clampUp(timeDay.offset(mondayBoundary, -1.5)),
            ).toEqual(mondayBoundary);
        });

        it('should not clamp up at the boundary of a discontinuity', () => {
            // clamping the boundary should return the bundary
            expect(skipWeekends().clampUp(mondayBoundary)).toEqual(
                mondayBoundary,
            );

            // by subtracting one day and one millisecond, this will fall just outside of the weekend, so should not be clamped
            const friday = timeMillisecond.offset(
                timeDay.offset(mondayBoundary, -2),
                -1,
            );
            expect(skipWeekends().clampUp(friday)).toEqual(friday);
        });
    });
    ...
    describe('clampDown', () => {
        ...
        it('should clamp down if given a weekend', () => {
            // clamping the boundary should return the bundary
            expect(skipWeekends().clampDown(saturdayBoundary)).toEqual(
                saturdayBoundary,
            );

            // try adding a millisecond
            expect(
                skipWeekends().clampDown(
                    timeMillisecond.offset(saturdayBoundary, 1),
                ),
            ...
            expect(
                skipWeekends().clampDown(
                    timeMinute.offset(saturdayBoundary, 1),
                ),
            ).toEqual(saturdayBoundary);

            // try adding a day
            expect(
                skipWeekends().clampDown(timeDay.offset(saturdayBoundary, 1)),
            ).toEqual(saturdayBoundary);

            // try adding one and a half days
            expect(
                skipWeekends().clampDown(timeDay.offset(saturdayBoundary, 1.5)),
            ).toEqual(saturdayBoundary);
        });
        ...
        it('should not clamp down at the boundary of a discontinuity', () => {
            ...
            expect(skipWeekends().clampDown(monday)).toEqual(monday);
    ...
    it('should remove weekends', () => {
        expect(skipWeekends().distance(mondayBoundary, wednesday)).toEqual(
            2 * millisPerDay,
        );
        expect(
            skipWeekends().distance(
                mondayBoundary,
                timeDay.offset(mondayBoundary, 7),
            ),
