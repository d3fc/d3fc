...
describe('skipUtcWeeklyPattern', () => {
    ...
    describe('clampUp', () => {
        it('should do nothing 1ms before non trading period', () => {
            const expected = utcMillisecond.offset(
    ...
    describe('clampDown', () => {
        ...
        it('should clamp down from Sunday 6:59:59.999pm to 1ms before Friday 13:20', () => {
            ...
            const actual = sut.clampDown(
                utcMillisecond.offset(sundayEndBoundary, -1),
            );
            expect(actual).toEqual(expected);
        });
    });
    ...
    describe('distance', () => {
        ...
        it('on DST boundaries should return 24 * 3600 * 1000', () => {
            ...
            expect(
                sut.distance(
                    new Date(Date.UTC(2022, 2, 27)),
                    new Date(Date.UTC(2022, 2, 28)),
                ),
            ).toBe(24 * 3600 * 1000);
            expect(
                sut.distance(
                    new Date(Date.UTC(2022, 9, 30)),
                    new Date(Date.UTC(2022, 9, 31)),
                ),
            ).toBe(24 * 3600 * 1000);
        });
    ...
    describe('offset', () => {
        it('0 offset should return same date', () => {
            expect(sut.offset(new Date(Date.UTC(2018, 0, 1)), 0)).toEqual(
                new Date(Date.UTC(2018, 0, 1)),
            );
        });

        it('-1ms offset should return end of previous day', () => {
            expect(sut.offset(new Date(Date.UTC(2018, 0, 1)), -1)).toEqual(
                new Date(Date.UTC(2017, 11, 31, 23, 59, 59, 999)),
            );
        });

        it('-1ms offset at the of non-trading period should return start of non-trading period', () => {
            expect(
                sut.offset(new Date(Date.UTC(2018, 0, 1, 8, 30)), -1),
            ).toEqual(new Date(Date.UTC(2018, 0, 1, 7, 44, 59, 999)));
        });

        it('-2ms offset should return end of previous day - 1ms', () => {
            expect(sut.offset(new Date(Date.UTC(2018, 0, 1)), -2)).toEqual(
                new Date(Date.UTC(2017, 11, 31, 23, 59, 59, 998)),
            );
        });

        it('-1ms offset after 1ms into trading day should return start of day', () => {
            expect(
                sut.offset(new Date(Date.UTC(2018, 0, 1, 0, 0, 0, 1)), -1),
            ).toEqual(new Date(Date.UTC(2018, 0, 1)));
        });

        it('should return start of next day when offset is 24hr', () => {
            const sut = skipUtcWeeklyPattern(tradingWeekWithoutDiscontinuities);
            expect(
                sut.offset(new Date(Date.UTC(2018, 0, 1)), 24 * 3600 * 1000),
            ).toEqual(new Date(Date.UTC(2018, 0, 2)));
        });

        it('should return start of previous day when offset is -24hr', () => {
            const sut = skipUtcWeeklyPattern(tradingWeekWithoutDiscontinuities);
            expect(
                sut.offset(new Date(Date.UTC(2018, 0, 2)), -24 * 3600 * 1000),
            ).toEqual(new Date(Date.UTC(2018, 0, 1)));
        });

        it('1ms offset at start of non-trading period should return end of non-trading period', () => {
            expect(
                sut.offset(new Date(Date.UTC(2018, 0, 1, 7, 45)), 1),
            ).toEqual(new Date(Date.UTC(2018, 0, 1, 8, 30, 0, 1)));
        });
        ...
        it('should return start of next week when offset is totalTradingWeekMilliseconds', () => {
            ...
            ).toEqual(new Date(Date.UTC(2018, 0, 8)));
        });
        ...
        it('should return start of 53rd week when offset is 52 * totalTradingWeekMilliseconds', () => {
            ...
            ).toEqual(new Date(Date.UTC(2018, 11, 31)));
        });

        it('on a DST Sunday boundary should return 1h into next trading period when offset is 1h', () => {
            expect(
                sut.offset(new Date(Date.UTC(2022, 2, 27, 1)), 3600 * 1000),
            ).toEqual(new Date(Date.UTC(2022, 2, 27, 20)));
        });

        it('on a DST boundry day should return next day when offset is 24 hours', () => {
            const sut = skipUtcWeeklyPattern(tradingWeekWithoutDiscontinuities);
            expect(
                sut.offset(new Date(Date.UTC(2022, 2, 27)), 24 * 3600 * 1000),
            ).toEqual(new Date(Date.UTC(2022, 2, 28)));
        });

        it('should return next day on a DST boundry when offset is 23 hours', () => {
            const sut = skipUtcWeeklyPattern({ Sunday: [['1:0', '2:0']] });
            expect(
                sut.offset(new Date(Date.UTC(2022, 2, 27)), 23 * 3600 * 1000),
            ).toEqual(new Date(Date.UTC(2022, 2, 28)));
        });

        it('should return end of second non-trading range', () => {
            const offset = utcMillisecond.count(
                new Date(Date.UTC(2018, 0, 1, 8, 30)),
                new Date(Date.UTC(2018, 0, 1, 13, 20)),
            );
            expect(
                sut.offset(new Date(Date.UTC(2018, 0, 1, 7, 45)), offset),
            ).toEqual(new Date(Date.UTC(2018, 0, 1, 19, 0, 0, 0)));
        });
