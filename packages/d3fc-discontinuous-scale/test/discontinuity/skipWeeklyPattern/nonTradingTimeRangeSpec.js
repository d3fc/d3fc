import {
    nonTradingTimeRange,
    standardiseTimeString,
} from '../../../src/discontinuity/skipWeeklyPattern/nonTradingTimeRange';
...
describe('nonTradingTimeRange', () => {
    ...
    it('throws for argument that is not string[]', () => {
        expect(() =>
            nonTradingTimeRange([new Date(), new Date()], utcDateTimeUtility),
        ).toThrow();
    });
    ...
    it('throws for string[] argument with invalid time string', () => {
        expect(() =>
            nonTradingTimeRange(['', ''], utcDateTimeUtility),
        ).toThrow();
    });

    it('should return lenght = 1 for SOD and "00:00:00.001" ', () => {
        expect(
            nonTradingTimeRange(['SOD', '00:00:00.001'], utcDateTimeUtility)
                .lenghtInMs,
        ).toEqual(1);
        expect(
            nonTradingTimeRange(['SOD', '00:00:00.001'], localDateTimeUtility)
                .lenghtInMs,
        ).toEqual(1);
    });
    ...
    it('should return total milliseconds per day for timeRange [SOD, EOD)', () => {
        expect(
            nonTradingTimeRange(['SOD', 'EOD'], utcDateTimeUtility).lenghtInMs,
        ).toEqual(1000 * 60 * 60 * 24);
        expect(
            nonTradingTimeRange(['SOD', 'EOD'], localDateTimeUtility)
                .lenghtInMs,
        ).toEqual(1000 * 60 * 60 * 24);
    });
});
