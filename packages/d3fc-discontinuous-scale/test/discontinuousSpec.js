...
describe('discontinuous', () => {
    ...
    it('should support copy', () => {
        ...
        expect(clone.discontinuityProvider()).toEqual(
            dateTime.discontinuityProvider(),
        );
    ...
    describe('tick calculations', () => {
        it('should ensure ticks are not within discontinuities (time)', () => {
            ...
            var ticksInDiscontinuityRange = scale
                .ticks()
                .filter(
                    (tick) =>
                        tick.getDay() === 6 /* Sat */ ||
        ...
        it('should support arguments being passed to ticks', () => {
            var scale = scaleLinear().domain([0, 100]);

            var scaleDiscontinuous = discontinuous(scaleLinear())
                .discontinuityProvider(discontinuityRange([25, 75]))
                .domain([0, 100]);

            expect(scaleDiscontinuous.ticks(100).length).toEqual(
                scale.ticks(100).length,
            );
        });
    });
    ...
    describe('without discontinuities', () => {
        ...
        var referenceScale = scaleTime().domain([start, end]).range(range);
        ...
        it('should clamp the resulting domain', () => {
            var scale = discontinuous(scaleLinear())
                .discontinuityProvider(
                    discontinuityRange([-0.1, 0.1], [9.9, 10.1]),
                )
                .domain([0.2, 9.8]);
