...
import {
    CanvasPlotAreaComponent,
    CartesianChart,
    SvgPlotAreaComponent,
    WebglPlotAreaComponent,
} from '../src/cartesian';
...
expectType<CartesianChart<ScaleLinear<number, number, never>, ScaleIdentity>>(
    chartCartesian(d3.scaleLinear()),
);

expectType<
    CartesianChart<
        d3.ScaleIdentity<never>,
        d3.ScaleLinear<number, number, never>
    >
>(chartCartesian(undefined, d3.scaleLinear()));
...
expectType<
    ...
    >
>(chartCartesian(d3.scaleLinear(), scaleTime()));

// Can accept a configuration object with one scale
expectType<
    CartesianChart<d3.ScaleIdentity, d3.ScaleLinear<number, number, never>>
>(
    chartCartesian({
        yScale: scaleLinear(),
    }),
