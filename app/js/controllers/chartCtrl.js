(function(d3, fc) {
	...
	chartCtrl = function chartCtrl($rootScope) {
        ...
        this.initialiseRSI = function(data) {
            ...
            share.rsiXScale = d3.time.scale().domain([share.minDate, share.maxDate]).range([0, rsiWidth]);
		...
		this.initialiseNavigator = function(data) {
		    ...
		    share.navXScale = d3.time.scale().domain([share.minDate, share.maxDate]).range([0, navWidth]);
	    ...
	    this.showHideFeatures = function() {
	    	...
	    	share.plotArea.selectAll('.volume').style('display', share.showVolume ? 'block' : 'none' );
