(function(d3, fc) {
    ...
    var container = d3.select('#stacked-line')
        .append('svg')
        .attr('width', width)
        .attr('height', height);
    ...
    function renderChart(data, offset, yDomain) {
        ...
        var stackedBar = fc.series.stacked.line()
            .xScale(x)
            .yScale(y)
            .xValue(function(d) { return d.state; })
            .decorate(function(sel, data, index) {
                sel.style('stroke', function(d, i) {
                    return color(index);
                });
            });
        ...
        function pixelSnap(stackedData, xPixel, yPixel) {
            ...
            // create an array of y pixel locations for each stacked line
