...
function createPlotArea(name, useVolumeScale, leftAlignPriceScale) {
	...
	var chartScale = {
		dateFrom: new Date(d3.min(dataSeries1, function (d) { return d.date; }).getTime() - 8.64e7),
		dateTo: new Date(d3.max(dataSeries1, function (d) { return d.date; }).getTime() + 8.64e7),
		priceFrom: d3.min(dataSeries1, function (d) { return d.low; }),
		priceTo: d3.max(dataSeries1, function (d) { return d.high; }),
		volumeFrom: d3.min(dataSeries1, function (d) { return d.volume; }),
		volumeTo: d3.max(dataSeries1, function (d) { return d.volume; }),
	};
