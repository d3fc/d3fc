...
var dataSeries2 = fc.utilities.dataGenerator()
	.randomSeed(dataSeed + 1)
	.seedDate(new Date(2014, 1, 1))
	.generate(60);

var dataSeries3 = fc.utilities.dataGenerator()
	.randomSeed(dataSeed + 2)
	.seedDate(new Date(2014, 1, 1))
	.generate(60);
...
var dataSeries_Large = fc.utilities.dataGenerator()
	.mu(0.2)
	.sigma(0.2)
	.randomSeed(dataSeed + 4)
	.seedDate(new Date(2014, 1, 1))
	.generate(365);


function createPlotArea(dataSeries, name, useVolumeScale, leftAlignPriceScale, userPercentageScale, useNoneFinancialX, justifyEnds) {
	...
	var dateScale = useNoneFinancialX ? 
		...
		fc.scale.dateTime()
			.hideWeekends(true)
			.padEnds(!justifyEnds)
			.domainFromValues(dataSeries, ['date'])
			.range([0, chartLayout.getPlotAreaWidth()]);

	var priceScale = fc.scale.linear()
		.domainFromValues(dataSeries,['high', 'low'])
		.nice()
		.range([chartLayout.getPlotAreaHeight(), 0]);

	var volumeScale = fc.scale.linear()
		.domainFromValues(dataSeries,['volume'])
		.nice()
		.range([chartLayout.getPlotAreaHeight(), 0]);
	...
	if( useVolumeScale ) {
		...
		chart.append('g')
			...
			.attr('transform', 'translate(' + (leftAlignPriceScale ? 0 : chartLayout.getPlotAreaWidth()) + ',0)')
			.call(percentageAxis);
	}
	else {
		chart.append('g')
