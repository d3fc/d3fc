...
With d3fc our aim is to enhance D3 by providing a suite of powerful charting components that make it easy for you to assemble exactly the chart you want. These components 'speak the language' of D3, supporting selections and datajoins. Furthermore, using the decorate pattern, you can enhance and manipulate the way in which they are rendered.

With d3fc components you can quickly assemble bespoke charts, without hiding the power of D3.

## Creating the data

Typically chart data will be supplied by some external source, in JSON or CSV format. For the purposes of this example we'll just generate some random data. d3fc has a number of `data` utilities, including components that help construct realistic ransom data. For this example we'll use the [random walk component](/components/data/walk.html) to generate 12 datapoints, mapping them to create datapoints with `sales` and `month` properties:
...
The current y-domain is a bit of a guess, being hard-coded to a range `[0, 20]`. Ideally the domain should be computed based on the data. In this case the domain should accommodate the greatest sales value, plus some padding, and also extent to zero.

There's another d3fc component that can help here, the [extent](/components/util/extent.html) utility. The following code ensures that zero is included, and adds a small amount of padding to the 'top' end of the range:
...
In this case, decorate is used to locate 'left handle' of the line annotation, to add the text label, and to change the fill property of the path elements used to render the bar series.
...
Here's the complete example, with the `render` function invoked when the browser windows resizes.

```js
{{{ building-a-chart-complete-js }}}
```
